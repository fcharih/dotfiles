...
#result = sp.run(f"op item list --format=json --tags ssh --session {session_token} | op item get - --fields 'private key,public key' --reveal --format=json --session {session_token}", shell=True, text=True, capture_output=True)
    result = sp.run(f"op item list --format=json --tags ssh | op item get - --fields 'private key,public key' --reveal --format=json", shell=True, text=True, capture_output=True)
    ...
    print("Keys successfully downloaded!")
except Exception as e:
    print("Keys could not be downloaded:", str(e))
